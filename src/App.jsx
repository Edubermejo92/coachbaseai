...
const DICT = {
  ...
  fr: {
    ...
    "ex.added": "✓ Ajouté", "ex.preview": "Aperçu", "ex.previewNote": "Aperçu du tableau · non modifiable", "ex.pickToPreview": "Choisissez un exercice pour le voir sur le tableau", "co.title": "Coach AI — assistant du staff technique", "co.thinking": "Coach AI réfléchit…", "co.placeholder": "Posez une question à Coach AI…", "co.send": "Envoyer", "co.sug1": "Qui devrait jouer plus de minutes la prochaine journée ?", "co.sug2": "Préparez-moi un entraînement de 75' sur les transitions", "co.sug3": "Analysez le match à partir des événements enregistrés", "co.error": "Impossible de se connecter à l'assistant. Réessayez.", "co.welcome1": "Bonjour, {name}. Je suis Coach AI ({club} · {team}). Votre rôle est {role}", "co.welcomeFree": " · forfait gratuit", "co.welcome2": ". Comment puis-je vous aider ?", "mp.title": "Panneau Master", "mp.fullAccess": "accès total", "mp.uniqueNote": "Ce compte est unique et ne peut pas être partagé depuis l'application : le serveur vérifie l'e-mail avant d'attribuer le rôle.", "mp.statusTitle": "État de l'installation", "mp.club1": "club", "mp.clubN": "clubs", "mp.cat1": "catégorie", "mp.catN": "catégories", "mp.noCrest": "sans écusson", "mp.reload": "↻ Recharger", "mp.manageClubs": "Gérer les clubs", "mp.clubsTitle": "Clubs", "mp.onlyAccountTitle": "Seul ce compte peut", "mp.perm1": "Créer et supprimer des clubs. Les catégories (U16 A, U14 B…) sont ajoutées par chaque utilisateur dans le sien.", "mp.perm2": "Téléverser l'écusson du club et l'appliquer à toutes ses catégories en une fois.", "mp.perm3": "Fixer la limite de places du staff technique de chaque club.", "mp.perm4": "Créer un compte directeur sportif.", "acc.button": "Accessibilité", "acc.title": "Accessibilité", "acc.zoom": "Taille du texte", "acc.brightness": "Luminosité", "acc.reset": "Réinitialiser", "acc.decrease": "Diminuer", "acc.increase": "Augmenter", "c.closeBtn": "Fermer", "pm.rival": "Adversaire", "pm.alsoUsesApp": "utilise aussi COACHBASE", "pm.historyTitle": "Historique", "pm.saveResult": "+ Enregistrer ce résultat", "pm.noHistory": "Vous n'avez encore enregistré aucun résultat. Enregistrez celui-ci pour commencer l'historique de la catégorie.", "pm.win": "G", "pm.draw": "N", "pm.loss": "P", "pm.inNMatches": "en {n} match(s)", "pm.against": "Contre {rival} :", "pm.firstSaved": "Premier match enregistré contre {rival}.", "pm.savedTitle": "Analyses enregistrées", "pm.noAnalysisYet": "Ce match n'a pas encore d'analyse enregistrée.", "tr.publishedTitle": "Séances publiées", "tr.next": "Prochaine", "tr.load": "Charger", "tr.pendingProposal": "⏳ Vous avez une proposition d'entraînement en attente d'approbation", "tr.sendProposal": "Envoyer la proposition à l'entraîneur", "tr.savedNamed": "✓ « {name} » enregistrée.", "tr.alreadyPending": "Vous avez déjà une proposition d'entraînement en attente d'approbation.", "tr.proposalSent": "✓ Proposition d'entraînement envoyée. En attente d'approbation.", "tr.sessionSaved": "✓ Séance de {n} min enregistrée.", "pm.genFail": "Impossible de générer l'analyse. Réessayez.", "tr.emptyTemplate": "Ce modèle est vide.", "tr.loadedSession": "✓ Séance du « {x} » chargée.", "tr.loadedTemplate": "✓ « {name} » chargé. Ajustez la date et l'heure.", "tr.summaryHeader": "ENTRAÎNEMENT", "tr.summaryTotal": "Durée totale :", "tr.summaryMaterial": "Matériel :", "st.matchByMatch": "Match par match", "st.noMatchLog": "Aucun match avec feuille de match pour l'instant. Enregistrez le match en Mode match et sauvegardez-le dans l'historique : il apparaîtra ici, avec ce qu'a fait chaque joueur.", "st.tapMatch": "Touchez un match pour voir la fiche de chaque joueur et la feuille de match.", "st.colPlayer": "Joueur", "st.colStarter": "Titulaire", "st.colGoals": "Buts", "st.colCards": "Cartons", "st.colSubs": "Remplacements", "st.colCalled": "Convoqué", "st.seasonTitle": "Saison · {n} match(s)", "tm.title": "Clubs et catégories", "tm.hintA": "Le club est le niveau supérieur — le Chamartín Vergara —. U16 A ou U14 B sont ses ", "tm.hintBold": "catégories", "tm.hintB": ", pas des clubs à part. Seul vous créez les clubs ; chaque utilisateur ajoute les catégories dans le sien.", "tm.newClub": "+ Nouveau club", "tm.namePh": "Nom du club. Ex. Chamartín Vergara", "tm.comunidadPh": "Région. Ex. Région de Madrid", "tm.createClub": "Créer le club", "tm.clubExists": "Ce club existait déjà ; celui-ci est réutilisé.", "tm.clubCreated": "✓ Club créé. Ajoutez maintenant ses catégories.", "tm.onlyMasterCreate": "Seul le Master peut créer des clubs.", "tm.clubCreateFail": "Impossible de créer le club.", "tm.noClubsYet": "Aucun club pour l'instant.", "tm.addCat": "+ Ajouter une catégorie", "tm.deleteClubConfirm": "Supprimer le club {club} ? Possible uniquement car il n'a aucune catégorie ni personne dedans.", "tm.deleteClub": "Supprimer le club", "tm.clubDeleted": "✓ Club supprimé.", "tm.notEmptyDelete": "Impossible : le club a {cat} catégorie(s) et {per} personne(s). Videz-le d'abord.", "tm.clubDeleteFail": "Impossible de supprimer le club.", "tm.crestToAll": "⬡ Écusson du club → toutes ses catégories", "tm.crestOverwriteConfirm": "Certaines catégories de {club} ont déjà leur propre écusson. Les remplacer aussi ?", "tm.crestApplied": "✓ Appliqué à {n} catégorie(s).", "tm.onlyMasterAction": "Seul le Master peut faire cela.", "tm.uploadFail": "Impossible de téléverser.", "tm.manageClub": "⚙ Gérer le club", "tm.uploading": "Téléversement…", "tm.checkingSeats": "Vérification des places et du paiement…", "tm.checkFail": "Impossible de vérifier. Réessayez.", "tm.seatsTitle": "Places du staff technique", "tm.occupiedLbl": "occupées", "tm.activeLbl": "actives", "tm.pendingLbl": "en attente", "tm.ofLimit": "sur {n}", "tm.noLimit": "· sans limite", "tm.saveLimit": "Enregistrer la limite", "tm.limitPh": "Sans limite", "tm.limitHint": "Laissez vide pour ne pas limiter (Club L).", "tm.paymentStatus": "Statut de paiement", "tm.upToDate": "À jour", "tm.notActive": "Non activé", "tm.renews": "se renouvelle", "tm.ends": "se termine", "tm.noSubscription": "Aucun abonnement actif parmi les e-mails de ce club.", "tm.crestBtn": "⬡ Écusson", "tm.crestUpdated": "✓ Écusson mis à jour.", "tm.crestUploadFail": "Impossible de téléverser l'écusson.", "tm.playersBtn": "◉ Joueurs", "tm.demoSquadNote": "L'effectif de chaque catégorie appartient à ses familles et joueurs : la démo montre le catalogue des clubs, pas leurs données réelles.", "tm.editBtn": "Modifier", "tm.demoWorkNote": "Travailler avec une vraie catégorie relève du véritable compte Master ; dans la démo vous pouvez parcourir le catalogue, pas y entrer.", "tm.inUse": "En cours d'utilisation", "tm.workWithThis": "Travailler avec celle-ci", "tm.loadingSquad": "Chargement de l'effectif…", "tm.squadLoadFail": "Impossible de charger l'effectif de cette catégorie.", "tm.squadEmpty": "Cette catégorie n'a pas encore de joueurs inscrits.", "tm.player1": "joueur", "tm.playerN": "joueurs", "tm.editCatTitle": "Modifier {name}", "tm.newCatTitle": "Nouvelle catégorie", "tm.newCatOf": " pour {club}", "tm.catNameLbl": "Nom de la catégorie", "tm.catNamePh": "U16 A", "tm.clubOfLbl": "Club auquel elle appartient", "tm.pickClub": "Choisissez un club…", "tm.teamWebLbl": "Site web de l'équipe", "tm.mapsLbl": "Google Maps du terrain", "tm.pickClubFirst": "Choisissez à quel club appartient la catégorie.", "tm.saved": "✓ Enregistré.", "tm.saveFail": "Impossible d'enregistrer. Vérifiez votre connexion.", "tm.crestNote": "L'écusson se téléverse depuis la liste ci-dessus avec le bouton ⬡ Écusson, une fois la catégorie créée.", "tm.categoryLbl": "Catégorie", "tm.f11": "Foot à 11", "tm.f7": "Foot à 7", "tm.noClub": "Sans club", "tm.noName": "Sans nom", "d.title": "Module de comportement", "d.desc1a": "Registre des incidents selon le ", "d.codeName": "Code disciplinaire", "d.desc1b": " de l'équipe. Les conséquences indiquées sont celles, littérales, du document signé par le club.", "d.note1": "Le délégué l'enregistre ; le staff technique le valide. Le document précise que chaque situation est évaluée individuellement par le staff technique et que les mesures ont toujours un caractère éducatif.", "d.newIncidentBtn": "+ Enregistrer un incident", "d.collectiveMeasureBtn": "⚖ Mesure collective", "d.viewNormBtn": "📑 Voir le règlement et les signatures", "d.exportCsvBtn": "⤓ Exporter en CSV", "d.unsignedWarn": "⚠ {n} joueur(s) n'ont pas encore signé le code disciplinaire. Aucune sanction financière ne doit être appliquée sans signature.", "d.newTitle": "Nouvel incident", "d.player": "Joueur", "d.pickPlayer": "Choisissez un joueur…", "d.date": "Date", "d.context": "Contexte", "d.normViolated": "Règle enfreinte", "d.minorFouls": "Fautes légères", "d.majorFouls": "Fautes graves", "d.cardLabel": "Carton", "d.economicSanction": "Sanction financière (facultatif)", "d.fault": "Faute", "d.consequenceLabel": "Conséquence selon le règlement :", "d.measuresApplied": "Mesures appliquées (les 8 du document)", "d.whatHappenedPh": "Ce qui s'est passé (faits, sans jugement de valeur)", "d.noEconomicWarnA": "⚠ Le code disciplinaire de l'équipe ", "d.noEconomicBold": "ne prévoit pas de sanctions financières", "d.noEconomicWarnB": " : toutes ses mesures sont éducatives. N'utilisez ce champ que si le club dispose d'un régime financier approuvé et signé.", "d.notSignedExtra": "De plus, ce joueur n'a pas signé le code.", "d.saveIncidentBtn": "Enregistrer l'incident", "d.teamTitle": "Mesure collective", "d.teamDesc": "Applique la même mesure à plusieurs joueurs à la fois. Un incident est créé par joueur, regroupés dans un lot que vous pouvez annuler entièrement ensuite.", "d.whoApplies": "À qui s'applique-t-elle ?", "d.amountPerPlayer": "Montant par joueur (facultatif)", "d.measuresApplied2": "Mesures appliquées", "d.teamReasonPh": "Motif de la mesure collective (ce qui s'est passé et pourquoi cela concerne tout le groupe)", "d.youthWarnA": "⚠ Vous allez appliquer ", "d.youthWarnB": " au total à {n} mineurs. Le code disciplinaire de l'équipe ne prévoit pas de sanctions financières, et une amende collective punit également ceux qui n'ont rien fait. Ne l'utilisez que si le club dispose d'un régime financier approuvé et signé par les familles.", "d.teamPotLine": "Cagnotte de l'équipe : {amount} € au total ({n} joueurs).", "d.applyToN": "Appliquer à {n} joueur(s)", "d.noMatchCriteria": "Aucun joueur ne correspond à ce critère.", "d.quickTitle": "Faire l'appel et enregistrement rapide", "d.presentN": "présents", "d.latesN": "retards", "d.absencesN": "absences", "d.markAllPresentBtn": "✓ Marquer tout le monde présent", "d.clearDayBtn": "Effacer la liste du jour", "d.faultsParen": "absence(s)", "d.latesParen": "retard(s)", "d.incidentsParen": "incident(s)", "d.unsignedTag": "⚠ non signé", "d.conductBtn": "⚠ Comportement", "d.otherFull": "Autre… (formulaire complet)", "d.autoNote": "Marquer Retard ou Absence crée automatiquement l'incident correspondant du code (L1 ponctualité, L2 communication des absences), en attente de validation par le staff technique. Justifiée ne génère pas d'incident.", "d.seeAsistNote": "Ceci concerne les retards et absences avec conséquence. Si vous voulez juste savoir qui est venu aujourd'hui et pourquoi — études, maladie, blessure — sans ouvrir d'incident, utilisez", "d.historyTitle": "Historique des incidents", "d.allPlayersOpt": "Tous les joueurs", "d.noIncidentsFilter": "Aucun incident avec ce filtre.", "d.pendingValidateTag": "En attente de validation", "d.paidTag": "payée", "d.condonedTag": "annulée", "d.pendingTag": "en attente", "d.markPaidBtn": "Marquer comme payée", "d.condoneBtn": "Annuler", "d.famInformedYes": "✓ Famille informée", "d.famInformedNo": "Marquer la famille comme informée", "d.batchTag": "Lot collectif", "d.validateBtn": "Valider", "d.annulBtn": "Annuler", "d.annulBatchBtn": "Annuler tout le lot", "d.filterAll": "Tous", "d.filterPend": "En attente de validation ({n})", "d.filterValidated": "Validés", "d.filterAnnulled": "Annulés", "d.summaryTitle": "Résumé par joueur", "d.noIncidentsReg": "Aucun incident enregistré.", "d.playerCol": "Joueur", "d.pendingCol": "En attente", "d.notSignedTitle": "N'a pas signé le code", "d.reiterationBtn": "G5 récidive", "d.reiterationTitle": "Récidive de fautes légères — le code la qualifie de faute grave G5", "d.reiterationDesc": "Récidive : {n} fautes légères accumulées.", "d.economicTitle": "Sanctions financières", "d.teamPotTitle": "Cagnotte de l'équipe", "d.pendingCollect": "en attente de paiement", "d.alreadyPaid": "déjà payé", "d.youthFooter": "Rappelez-vous que le code disciplinaire de cette équipe ne prévoit pas d'amendes : ses huit mesures sont éducatives. Si le club facture des montants, ils doivent figurer dans un règlement signé par les familles.", "d.autoLateDesc": "Retard enregistré lors de l'appel.", "d.autoAbsentDesc": "Absence non signalée enregistrée lors de l'appel.", "d.quickRegSuffix": " (enregistrement rapide).", "d.attOk": "Présent", "d.attLate": "Retard", "d.attAbsent": "Absence", "d.attJust": "Justifiée", "d.tgAll": "Tout l'effectif", "d.tgAbsent": "Étaient absents ce jour-là", "d.tgLate": "Sont arrivés en retard ce jour-là", "d.tgUnsigned": "N'ont pas signé le code", "d.tgManual": "Sélection manuelle", "d.gLeve": "légère", "d.gGrave": "grave", "doc.title": "Règlement du club", "doc.signLabel": "Signature", "doc.confirmLabel": "Confirmation", "doc.viewPdf": "Voir le PDF", "doc.download": "Télécharger", "doc.publishedOn": "Publié le {date} · version {v}", "doc.requiredSuffix": " · signature obligatoire", "doc.playersExTitle": "Exercices · joueurs", "doc.playersSignTitle": "Signatures · joueurs et familles", "doc.doneEx": "réalisés", "doc.doneSign": "signées", "doc.pending": "En attente", "doc.staffExTitle": "Confirmation · staff technique", "doc.staffSignTitle": "Signatures · staff technique", "doc.staffDoneEx": "confirmés", "doc.staffDoneSign": "signées", "doc.sanctionsTitle": "Contrôle des sanctions", "doc.allSigned": "Tout l'effectif a signé le code disciplinaire.", "doc.someUnsigned": "{n} joueur(s) n'ont pas signé le code disciplinaire.", "doc.sanctionsNote": "Une mesure disciplinaire n'est applicable que si la personne connaît et a accepté la règle. Avant d'appliquer une sanction — surtout financière — vérifiez ici que la signature est enregistrée.", "doc.doneExercise": "✓ Réalisé", "doc.doneSigned": "✓ Signé",
  ...
  de: {
    ...
    "ex.added": "✓ Hinzugefügt", "ex.preview": "Vorschau", "ex.previewNote": "Taktiktafel-Vorschau · nicht bearbeitbar", "ex.pickToPreview": "Wähle eine Übung, um sie auf der Taktiktafel zu sehen", "co.title": "Coach AI — Assistent für das Trainerteam", "co.thinking": "Coach AI denkt nach…", "co.placeholder": "Frag Coach AI…", "co.send": "Senden", "co.sug1": "Wer sollte am nächsten Spieltag mehr Minuten bekommen?", "co.sug2": "Bereite mir eine 75-minütige Einheit zu Umschaltmomenten vor", "co.sug3": "Analysiere das Spiel anhand der erfassten Ereignisse", "co.error": "Verbindung zum Assistenten fehlgeschlagen. Versuch es noch einmal.", "co.welcome1": "Hallo, {name}. Ich bin Coach AI ({club} · {team}). Deine Rolle ist {role}", "co.welcomeFree": " · Gratis-Plan", "co.welcome2": ". Womit kann ich helfen?", "mp.title": "Master-Bereich", "mp.fullAccess": "Vollzugriff", "mp.uniqueNote": "Dieses Konto ist einzigartig und kann nicht über die App weitergegeben werden: Der Server prüft die E-Mail, bevor die Rolle vergeben wird.", "mp.statusTitle": "Installationsstatus", "mp.club1": "Verein", "mp.clubN": "Vereine", "mp.cat1": "Altersklasse", "mp.catN": "Altersklassen", "mp.noCrest": "ohne Wappen", "mp.reload": "↻ Neu laden", "mp.manageClubs": "Vereine verwalten", "mp.clubsTitle": "Vereine", "mp.onlyAccountTitle": "Nur dieses Konto kann", "mp.perm1": "Vereine erstellen und löschen. Altersklassen (U16 A, U14 B…) fügt jeder Nutzer in seinem eigenen Verein hinzu.", "mp.perm2": "Das Vereinswappen hochladen und es auf alle Altersklassen gleichzeitig anwenden.", "mp.perm3": "Das Platzlimit für das Trainerteam jedes Vereins festlegen.", "mp.perm4": "Einen Sportdirektor anlegen.", "acc.button": "Barrierefreiheit", "acc.title": "Barrierefreiheit", "acc.zoom": "Textgröße", "acc.brightness": "Helligkeit", "acc.reset": "Zurücksetzen", "acc.decrease": "Verringern", "acc.increase": "Erhöhen", "c.closeBtn": "Schließen", "pm.rival": "Gegner", "pm.alsoUsesApp": "nutzt auch COACHBASE", "pm.historyTitle": "Verlauf", "pm.saveResult": "+ Dieses Ergebnis speichern", "pm.noHistory": "Du hast noch kein Ergebnis gespeichert. Speichere dieses und du beginnst mit dem Verlauf der Altersklasse.", "pm.win": "S", "pm.draw": "U", "pm.loss": "N", "pm.inNMatches": "in {n} Spiel(en)", "pm.against": "Gegen {rival}:", "pm.firstSaved": "Erstes gespeichertes Spiel gegen {rival}.", "pm.savedTitle": "Gespeicherte Analysen", "pm.noAnalysisYet": "Für dieses Spiel gibt es noch keine gespeicherte Analyse.", "tr.publishedTitle": "Veröffentlichte Einheiten", "tr.next": "Nächste", "tr.load": "Laden", "tr.pendingProposal": "⏳ Du hast einen Trainingsvorschlag, der auf Genehmigung wartet", "tr.sendProposal": "Vorschlag an den Trainer senden", "tr.savedNamed": "✓ „{name}\" gespeichert.", "tr.alreadyPending": "Du hast bereits einen Trainingsvorschlag, der auf Genehmigung wartet.", "tr.proposalSent": "✓ Trainingsvorschlag gesendet. Warte auf Genehmigung.", "tr.sessionSaved": "✓ {n}-Minuten-Einheit gespeichert.", "pm.genFail": "Die Analyse konnte nicht erstellt werden. Versuch es noch einmal.", "tr.emptyTemplate": "Diese Vorlage ist leer.", "tr.loadedSession": "✓ Einheit vom \"{x}\" geladen.", "tr.loadedTemplate": "✓ \"{name}\" geladen. Passe Datum und Uhrzeit an.", "tr.summaryHeader": "TRAINING", "tr.summaryTotal": "Gesamtdauer:", "tr.summaryMaterial": "Material:", "st.matchByMatch": "Spiel für Spiel", "st.noMatchLog": "Noch keine Spiele mit Spielbericht. Erfasse das Spiel im Spielmodus und speichere es im Verlauf: von dort erscheint es hier, mit dem, was jeder Spieler gemacht hat.", "st.tapMatch": "Tippe auf ein Spiel, um die Übersicht jedes Spielers und den Spielbericht zu sehen.", "st.colPlayer": "Spieler", "st.colStarter": "Startelf", "st.colGoals": "Tore", "st.colCards": "Karten", "st.colSubs": "Wechsel", "st.colCalled": "Nominiert", "st.seasonTitle": "Saison · {n} Spiel(e)", "tm.title": "Vereine und Altersklassen", "tm.hintA": "Der Verein ist die oberste Ebene — der Chamartín Vergara —. U16 A oder U14 B sind seine ", "tm.hintBold": "Altersklassen", "tm.hintB": ", keine eigenständigen Vereine. Nur du legst Vereine an; die Altersklassen fügt jeder Nutzer in seinem eigenen hinzu.", "tm.newClub": "+ Neuer Verein", "tm.namePh": "Name des Vereins. Z. B. Chamartín Vergara", "tm.comunidadPh": "Region. Z. B. Region Madrid", "tm.createClub": "Verein anlegen", "tm.clubExists": "Dieser Verein existierte bereits; er wird weiterverwendet.", "tm.clubCreated": "✓ Verein angelegt. Füge jetzt seine Altersklassen hinzu.", "tm.onlyMasterCreate": "Nur der Master kann Vereine anlegen.", "tm.clubCreateFail": "Verein konnte nicht angelegt werden.", "tm.noClubsYet": "Noch kein Verein vorhanden.", "tm.addCat": "+ Altersklasse hinzufügen", "tm.deleteClubConfirm": "Verein {club} löschen? Nur möglich, weil er keine Altersklassen oder Personen enthält.", "tm.deleteClub": "Verein löschen", "tm.clubDeleted": "✓ Verein gelöscht.", "tm.notEmptyDelete": "Nicht möglich: Der Verein hat {cat} Altersklasse(n) und {per} Person(en). Zuerst leeren.", "tm.clubDeleteFail": "Verein konnte nicht gelöscht werden.", "tm.crestToAll": "⬡ Vereinswappen → auf alle Altersklassen anwenden", "tm.crestOverwriteConfirm": "Einige Altersklassen von {club} haben bereits ein eigenes Wappen. Auch dort ersetzen?", "tm.crestApplied": "✓ Auf {n} Altersklasse(n) angewendet.", "tm.onlyMasterAction": "Nur der Master kann das tun.", "tm.uploadFail": "Hochladen fehlgeschlagen.", "tm.manageClub": "⚙ Verein verwalten", "tm.uploading": "Wird hochgeladen…", "tm.checkingSeats": "Plätze und Zahlungsstatus werden geprüft…", "tm.checkFail": "Prüfung fehlgeschlagen. Versuch es noch einmal.", "tm.seatsTitle": "Plätze für das Trainerteam", "tm.occupiedLbl": "belegt", "tm.activeLbl": "aktiv", "tm.pendingLbl": "ausstehend", "tm.ofLimit": "von {n}", "tm.noLimit": "· kein Limit", "tm.saveLimit": "Limit speichern", "tm.limitPh": "Kein Limit", "tm.limitHint": "Leer lassen für kein Limit (Club L).", "tm.paymentStatus": "Zahlungsstatus", "tm.upToDate": "Aktuell", "tm.notActive": "Nicht aktiviert", "tm.renews": "verlängert sich", "tm.ends": "endet", "tm.noSubscription": "Kein aktives Abo unter den E-Mails dieses Vereins.", "tm.crestBtn": "⬡ Wappen", "tm.crestUpdated": "✓ Wappen aktualisiert.", "tm.crestUploadFail": "Wappen konnte nicht hochgeladen werden.", "tm.playersBtn": "◉ Spieler", "tm.demoSquadNote": "Der Kader jeder Altersklasse gehört ihren Familien und Spielern: Die Demo zeigt den Vereinskatalog, nicht deren echte Daten.", "tm.editBtn": "Bearbeiten", "tm.demoWorkNote": "Mit einer echten Altersklasse zu arbeiten ist Sache des echten Master-Kontos; in der Demo kannst du den Katalog ansehen, aber nicht hineingehen.", "tm.inUse": "In Benutzung", "tm.workWithThis": "Mit dieser arbeiten", "tm.loadingSquad": "Kader wird geladen…", "tm.squadLoadFail": "Kader dieser Altersklasse konnte nicht geladen werden.", "tm.squadEmpty": "Diese Altersklasse hat noch keine angemeldeten Spieler.", "tm.player1": "Spieler", "tm.playerN": "Spieler", "tm.editCatTitle": "{name} bearbeiten", "tm.newCatTitle": "Neue Altersklasse", "tm.newCatOf": " für {club}", "tm.catNameLbl": "Name der Altersklasse", "tm.catNamePh": "U16 A", "tm.clubOfLbl": "Zugehöriger Verein", "tm.pickClub": "Verein wählen…", "tm.teamWebLbl": "Website der Mannschaft", "tm.mapsLbl": "Google Maps des Platzes", "tm.pickClubFirst": "Wähle, zu welchem Verein die Altersklasse gehört.", "tm.saved": "✓ Gespeichert.", "tm.saveFail": "Konnte nicht gespeichert werden. Prüfe deine Verbindung.", "tm.crestNote": "Das Wappen wird über die Liste oben mit dem Button ⬡ Wappen hochgeladen, sobald die Altersklasse angelegt ist.", "tm.categoryLbl": "Kategorie", "tm.f11": "Fußball 11", "tm.f7": "Fußball 7", "tm.noClub": "Kein Verein", "tm.noName": "Kein Name", "d.title": "Verhaltensmodul", "d.desc1a": "Vorfallprotokoll gemäß dem ", "d.codeName": "Verhaltenskodex", "d.desc1b": " des Teams. Die angezeigten Konsequenzen entsprechen wörtlich dem vom Verein unterzeichneten Dokument.", "d.note1": "Der Betreuer erfasst es, das Trainerteam bestätigt es. Das Dokument legt fest, dass jede Situation individuell vom Trainerteam bewertet wird und die Maßnahmen stets erzieherischen Charakter haben.", "d.newIncidentBtn": "+ Vorfall erfassen", "d.collectiveMeasureBtn": "⚖ Kollektive Maßnahme", "d.viewNormBtn": "📑 Regelwerk und Unterschriften ansehen", "d.exportCsvBtn": "⤓ CSV exportieren", "d.unsignedWarn": "⚠ {n} Spieler haben den Verhaltenskodex noch nicht unterschrieben. Ohne Unterschrift darf keine finanzielle Sanktion verhängt werden.", "d.newTitle": "Neuer Vorfall", "d.player": "Spieler", "d.pickPlayer": "Spieler wählen…", "d.date": "Datum", "d.context": "Kontext", "d.normViolated": "Verstoß gegen", "d.minorFouls": "Leichte Verstöße", "d.majorFouls": "Schwere Verstöße", "d.cardLabel": "Karte", "d.economicSanction": "Finanzielle Sanktion (optional)", "d.fault": "Verstoß", "d.consequenceLabel": "Konsequenz laut Kodex:", "d.measuresApplied": "Angewendete Maßnahmen (alle 8 aus dem Dokument)", "d.whatHappenedPh": "Was ist passiert (Fakten, ohne Wertung)", "d.noEconomicWarnA": "⚠ Der Verhaltenskodex des Teams ", "d.noEconomicBold": "sieht keine finanziellen Sanktionen vor", "d.noEconomicWarnB": ": Alle seine Maßnahmen sind erzieherisch. Nutze dieses Feld nur, wenn der Verein eine genehmigte und unterschriebene Finanzregelung hat.", "d.notSignedExtra": "Außerdem hat dieser Spieler den Kodex nicht unterschrieben.", "d.saveIncidentBtn": "Vorfall speichern", "d.teamTitle": "Kollektive Maßnahme", "d.teamDesc": "Wendet dieselbe Maßnahme auf mehrere Spieler gleichzeitig an. Pro Spieler wird ein Vorfall angelegt, gebündelt in einem Stapel, den du später komplett widerrufen kannst.", "d.whoApplies": "Für wen gilt sie?", "d.amountPerPlayer": "Betrag pro Spieler (optional)", "d.measuresApplied2": "Angewendete Maßnahmen", "d.teamReasonPh": "Grund für die kollektive Maßnahme (was ist passiert und warum betrifft es die ganze Gruppe)", "d.youthWarnA": "⚠ Du wirst insgesamt ", "d.youthWarnB": " an {n} Minderjährige verhängen. Der Verhaltenskodex des Teams sieht keine finanziellen Sanktionen vor, und eine Kollektivstrafe bestraft gleichermaßen diejenigen, die nichts falsch gemacht haben. Nutze dies nur, wenn der Verein eine von den Familien genehmigte und unterschriebene Finanzregelung hat.", "d.teamPotLine": "Mannschaftskasse: {amount} € insgesamt ({n} Spieler).", "d.applyToN": "Auf {n} Spieler anwenden", "d.noMatchCriteria": "Kein Spieler erfüllt dieses Kriterium.", "d.quickTitle": "Anwesenheit erfassen und Schnellerfassung", "d.presentN": "anwesend", "d.latesN": "verspätet", "d.absencesN": "abwesend", "d.markAllPresentBtn": "✓ Alle als anwesend markieren", "d.clearDayBtn": "Tagesliste löschen", "d.faultsParen": "Fehltermin(e)", "d.latesParen": "Verspätung(en)", "d.incidentsParen": "Vorfall/Vorfälle", "d.unsignedTag": "⚠ nicht unterschrieben", "d.conductBtn": "⚠ Verhalten", "d.otherFull": "Andere… (vollständiges Formular)", "d.autoNote": "Die Markierung von Verspätung oder Fehlen erzeugt automatisch den entsprechenden Vorfall aus dem Kodex (L1 Pünktlichkeit, L2 Mitteilung von Abwesenheiten), zur Bestätigung durch das Trainerteam. Entschuldigt erzeugt keinen Vorfall.", "d.seeAsistNote": "Das ist für Verspätungen und Fehlzeiten mit Konsequenz. Wenn du nur wissen willst, wer heute da war und warum — Schule, Krankheit, Verletzung — ohne einen Vorfall zu eröffnen, nutze", "d.historyTitle": "Vorfallverlauf", "d.allPlayersOpt": "Alle Spieler", "d.noIncidentsFilter": "Keine Vorfälle mit diesem Filter.", "d.pendingValidateTag": "Bestätigung ausstehend", "d.paidTag": "bezahlt", "d.condonedTag": "erlassen", "d.pendingTag": "ausstehend", "d.markPaidBtn": "Als bezahlt markieren", "d.condoneBtn": "Erlassen", "d.famInformedYes": "✓ Familie informiert", "d.famInformedNo": "Familie als informiert markieren", "d.batchTag": "Sammelvorgang", "d.validateBtn": "Bestätigen", "d.annulBtn": "Widerrufen", "d.annulBatchBtn": "Gesamten Stapel widerrufen", "d.filterAll": "Alle", "d.filterPend": "Bestätigung ausstehend ({n})", "d.filterValidated": "Bestätigt", "d.filterAnnulled": "Widerrufen", "d.summaryTitle": "Zusammenfassung pro Spieler", "d.noIncidentsReg": "Keine Vorfälle erfasst.", "d.playerCol": "Spieler", "d.pendingCol": "Ausstehend", "d.notSignedTitle": "Hat den Kodex nicht unterschrieben", "d.reiterationBtn": "G5 Wiederholung", "d.reiterationTitle": "Wiederholte leichte Verstöße — der Kodex stuft dies als schweren Verstoß G5 ein", "d.reiterationDesc": "Wiederholung: {n} angesammelte leichte Verstöße.", "d.economicTitle": "Finanzielle Sanktionen", "d.teamPotTitle": "Mannschaftskasse", "d.pendingCollect": "noch einzuziehen", "d.alreadyPaid": "bereits bezahlt", "d.youthFooter": "Denk daran, dass der Verhaltenskodex dieses Teams keine Geldstrafen vorsieht: seine acht Maßnahmen sind erzieherisch. Wenn der Verein Beträge erhebt, müssen sie in einem von den Familien unterschriebenen Regelwerk festgelegt sein.", "d.autoLateDesc": "Verspätung bei der Anwesenheitserfassung protokolliert.", "d.autoAbsentDesc": "Nicht gemeldete Abwesenheit bei der Anwesenheitserfassung protokolliert.", "d.quickRegSuffix": " (Schnellerfassung).", "d.attOk": "Anwesend", "d.attLate": "Verspätet", "d.attAbsent": "Fehlt", "d.attJust": "Entschuldigt", "d.tgAll": "Der gesamte Kader", "d.tgAbsent": "Waren an dem Tag abwesend", "d.tgLate": "Kamen an dem Tag zu spät", "d.tgUnsigned": "Haben den Kodex nicht unterschrieben", "d.tgManual": "Manuelle Auswahl", "d.gLeve": "leicht", "d.gGrave": "schwer", "doc.title": "Vereinsregelwerk", "doc.signLabel": "Unterschrift", "doc.confirmLabel": "Bestätigung", "doc.viewPdf": "PDF ansehen", "doc.download": "Herunterladen", "doc.publishedOn": "Veröffentlicht am {date} · Version {v}", "doc.requiredSuffix": " · Unterschrift erforderlich", "doc.playersExTitle": "Übungen · Spieler", "doc.playersSignTitle": "Unterschriften · Spieler und Familien", "doc.doneEx": "erledigt", "doc.doneSign": "unterschrieben", "doc.pending": "Ausstehend", "doc.staffExTitle": "Bestätigung · Trainerteam", "doc.staffSignTitle": "Unterschriften · Trainerteam", "doc.staffDoneEx": "bestätigt", "doc.staffDoneSign": "unterschrieben", "doc.sanctionsTitle": "Sanktionskontrolle", "doc.allSigned": "Der gesamte Kader hat den Verhaltenskodex unterschrieben.", "doc.someUnsigned": "{n} Spieler haben den Verhaltenskodex nicht unterschrieben.", "doc.sanctionsNote": "Eine disziplinarische Maßnahme ist nur durchsetzbar, wenn die Person die Regel kennt und akzeptiert hat. Bevor du eine Sanktion anwendest — insbesondere eine finanzielle — prüfe hier, dass die Unterschrift erfasst ist.", "doc.doneExercise": "✓ Erledigt", "doc.doneSigned": "✓ Unterschrieben",
...
/* Zoom y brillo de accesibilidad: preferencias del dispositivo, igual que el
   tema. El zoom usa la propiedad CSS `zoom` sobre <html> -bien soportada en
   los navegadores móviles (Chrome, Safari, Samsung Internet), que es donde
   vive este control- en vez de transform:scale(), que exigiría recalcular el
   ancho a mano para no dejar una franja en blanco. */
const ZOOM_KEY = "cb_zoom";
const BRILLO_KEY = "cb_brillo";
const ZOOM_MIN = 80, ZOOM_MAX = 150, ZOOM_STEP = 10;
const BRILLO_MIN = 60, BRILLO_MAX = 140, BRILLO_STEP = 10;
const numGuardado = (key, def, min, max) => {
  try {
...
export default function App() {
  ...
  /* Zoom y brillo de accesibilidad (botón flotante en móvil, ver AccesFAB
     más abajo). Mismo patrón que el tema: estado del dispositivo, aplicado
     directamente al <html> y guardado en localStorage. */
  const [zoomWeb, setZoomWeb] = useState(() => numGuardado(ZOOM_KEY, 100, ZOOM_MIN, ZOOM_MAX));
  const [brilloWeb, setBrilloWeb] = useState(() => numGuardado(BRILLO_KEY, 100, BRILLO_MIN, BRILLO_MAX));
  useEffect(() => {
    document.documentElement.style.zoom = zoomWeb === 100 ? "" : `${zoomWeb}%`;
    try { localStorage.setItem(ZOOM_KEY, String(zoomWeb)); } catch { /* sin almacenamiento */ }
  }, [zoomWeb]);
  useEffect(() => {
    document.documentElement.style.filter = brilloWeb === 100 ? "" : `brightness(${brilloWeb}%)`;
    try { localStorage.setItem(BRILLO_KEY, String(brilloWeb)); } catch { /* sin almacenamiento */ }
  }, [brilloWeb]);
  ...
  const trainSummary = () => {
    ...
    return `🏋️ *${t("tr.summaryHeader")} — ${session.club} ${session.team.name}*\n📅 ${fechaLegible(trainMeta.fecha, lang) || "—"} · ⏰ ${trainMeta.hora || "—"}\n🎯 ${trainMeta.objetivo || "—"}\n\n${lines.join("\n")}\n\n⏱ ${t("tr.summaryTotal")} ${trainTotal} min\n🎒 ${t("tr.summaryMaterial")} ${trainMaterials.join(", ") || "—"}`;
  };
  ...
  const guardarPlantilla = async () => {
    ...
    if (!session?.team?.rec) { setPlMsg(t("se.planNotCloud")); return; }
    ...
    setPlBusy(false);
    if (out?.ok) { setPlMsg(t("tr.savedNamed").replace("{name}", plNombre.trim())); setPlNombre(""); setPlCompartir(false); cargarPlantillas(); }
    else setPlMsg(t("se.planSaveFail"));
  };
  ...
  const guardarSesion = async () => {
    ...
    if (canProposeChanges()) {
      if (miPropuestaPendiente("training")) { setSesMsg(t("tr.alreadyPending")); return; }
      setSesBusy(true); setSesMsg("");
      await proposeChange("training", { meta: trainMeta, blocks: trainBlocks, target: trainTarget });
      setSesBusy(false);
      setSesMsg(t("tr.proposalSent"));
      setTimeout(() => setSesMsg(""), 5000);
      return;
    }
    if (!session?.team?.rec) { setSesMsg(t("se.planNotCloud")); return; }
    setSesBusy(true); setSesMsg("");
    const nombre = [fechaLegible(trainMeta.fecha, lang) || new Date().toLocaleDateString(lang === "es" ? "es-ES" : lang), trainMeta.hora, trainMeta.objetivo]
      .filter(Boolean).join(" · ");
    const out = await airPlantillaNueva({
    ...
    setSesBusy(false);
    setSesMsg(out?.ok ? t("tr.sessionSaved").replace("{n}", trainTotal) : t("se.planSaveFail"));
...
ACTA:\n${evTxt}`;
    try {
      ...
      setPostTxt(text || t("pm.genFail"));
    } catch {
      setPostTxt(t("co.error"));
    }
    setPostBusy(false);
  };
...
ACTA:\n${evTxt}`;
  ...
  const usarPlantilla = async (p) => {
    ...
    if (!Array.isArray(bloques) || !bloques.length) { setPlMsg(t("tr.emptyTemplate")); return; }
    ...
    setPlMsg(p.plantilla === false ? t("tr.loadedSession").replace("{x}", p.fecha || p.nombre) : t("tr.loadedTemplate").replace("{name}", p.nombre));
...
PLANTILLA (disponibilidad):\n${roster}\nMARCADOR: ${score.us}-${score.them} | EVENTOS:\n${evTxt}`;
  ...
  const renderMasterPanel = () => {
    ...
    return (
      <div className="space-y-4">
        <Card title={t("mp.title")}>
          <div className="flex items-start gap-3 flex-wrap">
            ...
            <div className="flex-1 min-w-[220px]">
              ...
              <div className="text-xs" style={{ color: C.dim }}>{session.email} · {t("mp.fullAccess")}</div>
        ...
        <Card title={t("mp.statusTitle")}>
          {teamMsg ? (
            <div className="text-sm" style={{ color: C.warn }}>{teamMsg}</div>
          ) : (
            <div className="flex flex-wrap gap-2">
              <Cifra n={clubes.length} txt={clubes.length === 1 ? t("mp.club1") : t("mp.clubN")} />
              <Cifra n={teams.length} txt={teams.length === 1 ? t("mp.cat1") : t("mp.catN")} />
              <Cifra n={sinEscudo} txt={t("mp.noCrest")} />
            </div>
          )}
          <div className="flex flex-wrap gap-2 mt-3">
            ...
            <button onClick={() => setTab("equipos")} className="text-sm px-4 py-2 rounded-lg font-display uppercase tracking-wide font-semibold" style={{ background: AC, color: C.sobre }}>{t("mp.manageClubs")}</button>
          </div>
        </Card>

        {clubes.length > 0 && (
          <Card title={t("mp.clubsTitle")}>
            <div className="flex flex-col gap-1.5">
              {clubes.map((g) => (
                <button key={g.key} onClick={() => setTab("equipos")}
                  ...
                  <span className="text-[11px] shrink-0" style={{ color: C.dim }}>{g.items.length} {g.items.length === 1 ? t("mp.cat1") : t("mp.catN")}</span>
                </button>
              ))}
            </div>
          </Card>
        ...
        <Card title={t("mp.onlyAccountTitle")}>
  ...
  const renderTeams = () => {
    ...
    return (
      <div className="space-y-4">
        <Card title={t("tm.title")}>
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            ...
            <button onClick={() => setClubNuevo({ nombre: "", comunidad: session.comunidad || "" })}
              className="text-sm px-4 py-2 rounded-lg font-display uppercase tracking-wide font-semibold" style={{ background: AC, color: C.sobre }}>{t("tm.newClub")}</button>
          </div>
          {clubNuevo && (
            <div className="rounded-lg border p-3 mb-3" style={{ borderColor: AC, background: C.panel2 }}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                <input value={clubNuevo.nombre} autoFocus onChange={(e) => setClubNuevo({ ...clubNuevo, nombre: e.target.value })}
                  placeholder={t("tm.namePh")} className="px-3 py-2 rounded-lg border bg-transparent" style={{ borderColor: C.line, color: C.chalk }} />
                <input value={clubNuevo.comunidad} onChange={(e) => setClubNuevo({ ...clubNuevo, comunidad: e.target.value })}
                  placeholder={t("tm.comunidadPh")} className="px-3 py-2 rounded-lg border bg-transparent" style={{ borderColor: C.line, color: C.chalk }} />
              </div>
              <div className="flex flex-wrap gap-2 mt-3">
                <button disabled={!clubNuevo.nombre.trim()} onClick={async () => {
                  ...
                  if (out?.ok) { setClubNuevo(null); setTeamMsg(out.reutilizado ? t("tm.clubExists") : t("tm.clubCreated")); loadTeams(); }
                  else setTeamMsg(out?.reason === "no_autorizado" ? t("tm.onlyMasterCreate") : t("tm.clubCreateFail"));
                }} className="text-sm px-4 py-2 rounded-lg font-display uppercase tracking-wide font-semibold disabled:opacity-40" style={{ background: AC, color: C.sobre }}>{t("tm.createClub")}</button>
          ...
          {teams.length === 0 && clubsAll.length === 0 && !teamMsg && <div className="text-sm" style={{ color: C.dim }}>{t("tm.noClubsYet")}</div>}
          {(() => {
            ...
            /* Se arranca de la lista de clubs para que los recién creados, que
               ...
               puedan añadir. */
            for (const c of clubsAll) porClub.push({ key: c.rec, clubRec: c.rec, club: c.name || t("tm.noName"), items: [] });
            for (const tm of teams) {
              const k = tm.clubRec || tm.club || "sin-club";
              let g = porClub.find((x) => x.key === k);
              if (!g) { g = { key: k, clubRec: tm.clubRec || null, club: tm.club || t("tm.noClub"), items: [] }; porClub.push(g); }
              g.items.push(tm);
            }
            return porClub.map((g) => (
              <div key={g.key} className="mb-4">
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  ...
                  <div className="font-display uppercase tracking-wide text-xs" style={{ color: C.dim }}>{g.items.length} {g.items.length === 1 ? t("mp.cat1") : t("mp.catN")}</div>
                  {g.clubRec && (
                    <button onClick={() => setTeamEdit({ rec: null, name: "", cat: "infantil", f7: false, club: g.club, clubRec: g.clubRec, comunidad: session.comunidad, web: "", maps: "" })}
                      className="text-[11px] px-2.5 py-1 rounded-lg border" style={{ borderColor: AC, color: AC }}>{t("tm.addCat")}</button>
                  )}
                  {g.clubRec && g.items.length === 0 && (
                    <button onClick={async () => {
                      if (!confirm(t("tm.deleteClubConfirm").replace("{club}", g.club))) return;
                      const out = await airBorrarClub(g.clubRec);
                      if (out?.ok) { setTeamMsg(t("tm.clubDeleted")); loadTeams(); }
                      else if (out?.reason === "no_vacio") setTeamMsg(t("tm.notEmptyDelete").replace("{cat}", out.categorias).replace("{per}", out.personas));
                      else setTeamMsg(t("tm.clubDeleteFail"));
                    }} className="text-[11px] px-2.5 py-1 rounded-lg border" style={{ borderColor: C.line, color: C.red }}>{t("tm.deleteClub")}</button>
                  )}
                  {g.clubRec && (
                    <label className="text-[11px] px-2.5 py-1 rounded-lg border cursor-pointer" style={{ borderColor: AC, color: AC }}>
                      ...
                      <input type="file" accept="image/*" className="hidden" onChange={(e) => {
                        ...
                        if (!f) return;
                        const forzar = g.items.some((it) => it.crest) && confirm(t("tm.crestOverwriteConfirm").replace("{club}", g.club));
                        ...
                        rd.onload = async () => {
                          ...
                          if (out?.ok) {
                            setClubCrestMsg((m) => ({ ...m, [g.key]: t("tm.crestApplied").replace("{n}", out.equiposActualizados) }));
                            loadTeams();
                          } else setClubCrestMsg((m) => ({ ...m, [g.key]: out?.reason === "no_autorizado" ? t("tm.onlyMasterAction") : t("tm.uploadFail") }));
                        };
                        rd.readAsDataURL(f);
                      }} />
                    </label>
                  ...
                  {clubCrestBusy === g.key && <span className="text-[11px]" style={{ color: C.dim }}>{t("tm.uploading")}</span>}
                ...
                {clubAdminOpen === g.key && (
                  <div className="rounded-lg border p-3 mb-3 text-sm" style={{ borderColor: AC, background: C.panel2 }}>
                    {clubAdminBusy && !clubAdminData[g.key] ? (
                      <div style={{ color: C.dim }}>{t("tm.checkingSeats")}</div>
                    ) : (() => {
                      const d = clubAdminData[g.key];
                      if (!d) return <div style={{ color: C.dim }}>{t("tm.checkFail")}</div>;
                      const s = d.suscripcion;
                      const activa = s && /activ/i.test(s.estado || "");
                      return (
                        <div className="space-y-3">
                          <div>
                            <div className="font-display uppercase tracking-wide text-xs mb-1" style={{ color: C.dim }}>{t("tm.seatsTitle")}</div>
                            <div className="flex flex-wrap items-center gap-2">
                              <span style={{ color: C.chalk }}>
                                {d.ocupadas} {t("tm.occupiedLbl")} ({d.activos} {t("tm.activeLbl")}, {d.pendientes} {t("tm.pendingLbl")})
                                {d.limite > 0 ? ` ${t("tm.ofLimit").replace("{n}", d.limite)}` : ` ${t("tm.noLimit")}`}
                              </span>
                            </div>
                            <div className="flex items-center gap-2 mt-2">
                              <input value={clubAdminLimite} onChange={(e) => setClubAdminLimite(e.target.value.replace(/[^0-9]/g, ""))}
                                placeholder={t("tm.limitPh")} className="w-28 px-2 py-1.5 rounded-lg border bg-transparent text-xs"
                                style={{ borderColor: C.line, color: C.chalk }} />
                              <button onClick={async () => {
                              ...
                              }} className="text-xs px-3 py-1.5 rounded-lg font-semibold" style={{ background: AC, color: C.sobre }}>{t("tm.saveLimit")}</button>
                              <span className="text-[10px]" style={{ color: C.dim }}>{t("tm.limitHint")}</span>
                            </div>
                          </div>
                          <div>
                            <div className="font-display uppercase tracking-wide text-xs mb-1" style={{ color: C.dim }}>{t("tm.paymentStatus")}</div>
                            {s ? (
                              <div className="flex flex-wrap items-center gap-2">
                                <span className="text-xs font-display uppercase tracking-wide px-2 py-1 rounded"
                                  ...
                                  {activa ? t("tm.upToDate") : s.estado || t("tm.notActive")}
                                </span>
                                <span className="text-xs" style={{ color: C.dim }}>{s.email}</span>
                                {s.periodoFin && <span className="text-xs" style={{ color: C.dim }}>· {s.cancelarFin ? t("tm.ends") : t("tm.renews")} {new Date(s.periodoFin).toLocaleDateString(lang === "es" ? "es-ES" : lang)}</span>}
                              </div>
                            ) : (
                              <span className="text-xs" style={{ color: C.red }}>{t("tm.noSubscription")}</span>
                            )}
                          </div>
                        </div>
                      );
              ...
              <div key={tm.rec} className="rounded-lg border p-3 flex flex-wrap items-center gap-3"
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[11px]" style={{ color: C.dim }}>{tm.club || t("tm.noClub")} · {tm.sub}</div>
                </div>
                <div className="flex flex-wrap gap-2 text-xs">
                    ...
                    <input type="file" accept="image/*" className="hidden" onChange={async (e) => {
                      ...
                      rd.onload = async () => {
                        ...
                        if (out?.url) {
                          ...
                          setTeamMsg(t("tm.crestUpdated"));
                        } else setTeamMsg(t("tm.crestUploadFail"));
                      };
                      rd.readAsDataURL(f);
                    }} />
                  </label>
                  <button onClick={() => (esMasterDemo ? setTeamMsg(t("tm.demoSquadNote")) : verPlantilla(tm.rec))}
                  ...
                  <button onClick={() => setTeamEdit({ ...tm })} className="px-3 py-1.5 rounded-lg border" style={{ borderColor: C.line, color: C.chalk }}>{t("tm.editBtn")}</button>
                  <button onClick={() => (esMasterDemo ? setTeamMsg(t("tm.demoWorkNote")) : (() => { setSession((sx) => ({ ...sx, team: tm, club: tm.club || sx.club, comunidad: tm.comunidad || sx.comunidad })); setCrest(tm.crest || null); setTab("inicio"); })())}
                    ...
                    {session.team?.rec === tm.rec ? t("tm.inUse") : t("tm.workWithThis")}
                  </button>
                </div>
                {plantillaAbierta === tm.rec && (
                  <div className="w-full mt-1 pt-3 border-t" style={{ borderColor: C.line }}>
                    {plantillasVistas[tm.rec] === undefined ? (
                      <div className="text-xs" style={{ color: C.dim }}>{t("tm.loadingSquad")}</div>
                    ) : !plantillasVistas[tm.rec] ? (
                      <div className="text-xs" style={{ color: C.red }}>{t("tm.squadLoadFail")}</div>
                    ) : plantillasVistas[tm.rec].length === 0 ? (
                      <div className="text-xs" style={{ color: C.dim }}>{t("tm.squadEmpty")}</div>
                    ) : (
                      <>
                        <div className="text-[11px] mb-2" style={{ color: C.dim }}>
                          {plantillasVistas[tm.rec].length} {plantillasVistas[tm.rec].length === 1 ? t("tm.player1") : t("tm.playerN")}
                        </div>
        ...
        {E && (
          <Card title={E.rec ? t("tm.editCatTitle").replace("{name}", E.name) : t("tm.newCatTitle") + (E.club ? t("tm.newCatOf").replace("{club}", E.club) : "")}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
              <label className="space-y-1">
                <div style={{ color: C.dim }}>{t("tm.catNameLbl")}</div>
                <input value={E.name} onChange={(e) => setTeamEdit({ ...E, name: e.target.value })} placeholder={t("tm.catNamePh")}
              ...
              <label className="space-y-1">
                ...
                <div style={{ color: C.dim }}>{t("tm.clubOfLbl")}</div>
            ...
            <div className="mt-3">
              <div className="text-xs mb-2" style={{ color: C.dim }}>{t("tm.categoryLbl")}</div>
              <div className="flex flex-wrap gap-2">
                {CATEGORIAS.map((c) => (
              ...
              <div className="flex gap-2 mt-2">
                {[[t("tm.f11"), false], [t("tm.f7"), true]].map(([l, v]) => (
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              <button onClick={async () => {
                ...
                if (!E.rec && !E.clubRec) { setTeamMsg(t("tm.pickClubFirst")); return; }
                const ok = E.rec
                ...
                setTeamMsg(ok ? t("tm.saved") : t("tm.saveFail"));
                setTeamEdit(null);
                if (ok) loadTeams();
  ...
  const guardarPlan = async () => {
    if (planBusy) return;
    if (!session?.team?.rec) { setPlanMsg(t("se.planNotCloud")); return; }
  ...
  /* ---- La planificación coincide con el calendario real ----
     Antes cada mes era una etiqueta suelta ("Septiembre", "Octubre"…) sin
     relación con los partidos de verdad. Aquí se calcula a qué año de
     calendario corresponde cada mes de LA TEMPORADA EN CURSO (temporada =
     de septiembre a junio, así que enero del año que viene sigue siendo
     "esta temporada"), y se cruza con fixtures para enseñar en cada mes
     los partidos reales que caen dentro. */
  const seasonStartYear = (() => {
    const hoy = new Date();
    const mesHoy = hoy.getMonth() + 1;
    return mesHoy >= 7 ? hoy.getFullYear() : hoy.getFullYear() - 1;
  })();
  const anioDelMes = (m) => (m >= 7 ? seasonStartYear : seasonStartYear + 1);
  const partidosDelMes = (m) => {
    const prefijo = `${anioDelMes(m)}-${String(m).padStart(2, "0")}`;
  ...
  /* ---- Sesión de 1 hora propuesta por la IA ----
     ...
     hay), le pide a Coach AI una sesión completa de 60 minutos en bloques,
...
La suma de todos los "dur" debe ser exactamente 60. Usa nombres de bloque en ${lang === "es" ? "español" : lang === "en" ? "inglés" : lang === "fr" ? "francés" : lang === "de" ? "alemán" : "portugués"}.`;
            ...
            return (
              <div key={m.k} className="rounded-lg border p-3" style={{ borderColor: d.pilares.length ? C.line : "rgba(54,69,79,0.07)", background: C.panel2 }}>
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  <span className="font-display uppercase tracking-wide text-sm w-28 shrink-0" style={{ color: C.chalk }}>{nombreMes}</span>
                  {PILARES.map((pl) => {
                ...
                {/* El calendario real, no una etiqueta suelta: cuántos partidos
                    hay ese mes y qué días, para planificar la carga con
                    los ojos abiertos en vez de a ciegas. */}
                <div className="text-[11px] mb-2" style={{ color: partidos.length ? AC : C.dim }}>
                  {partidos.length > 0
                    ? `📅 ${partidos.length} ${partidos.length === 1 ? t("se.match") : t("se.matches")}: ${partidos.map((f) => f.date.slice(8, 10) + "/" + f.date.slice(5, 7)).join(", ")}`
                    : `📅 ${t("se.noMatches")}`}
                </div>
                <input value={d.objetivo} onChange={(e) => setObjetivoMes(m.k, e.target.value)}
                  placeholder={t("se.goalPh")} aria-label={`${t("se.goal")} ${nombreMes}`}
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        ...
                        {props.length > 3 && (
                          <button onClick={() => setMesAbierto(abierto ? null : m.k)}
                            ...
                            {abierto ? t("se.seeLess") : t("se.seeMore").replace("{n}", props.length - 3)}
                          </button>
                        )}
                      </div>
                    </div>
  ...
  const renderPostMatch = () => (
    <div className="space-y-4">
      <Card title={t("pm.title")}>
        {(() => {
          const rivalTxt = matchInfo.rival || t("pm.rival");
          ...
          return (
            <>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-[11px] truncate" style={{ color: C.dim }}>
                      {eq ? `${eq.club || ""} · ${t("pm.alsoUsesApp")}` : t("pm.rival")}
                    </div>
                  </div>
                </div>
                <div className="text-xs" style={{ color: C.dim }}>
              ...
              <div className="rounded-lg border p-3 mb-3" style={{ borderColor: C.line, background: C.panel2 }}>
                <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                  <div className="font-display text-xs uppercase tracking-widest" style={{ color: C.dim }}>{t("pm.historyTitle")}</div>
                  {events.length > 0 && (
                ...
                {(() => {
                  ...
                  if (!historial.length) {
                    return <div className="text-[11px]" style={{ color: C.dim }}>{t("pm.noHistory")}</div>;
                  }
                  const bt = balance(historial), br = balance(contra);
                  return (
                    <>
                      <div className="flex flex-wrap gap-4 text-xs mb-2" style={{ color: C.chalk }}>
                        <span><strong style={{ color: C.green }}>{bt.g}</strong> {t("pm.win")} · <strong>{bt.e}</strong> {t("pm.draw")} · <strong style={{ color: C.red }}>{bt.p}</strong> {t("pm.loss")}</span>
                        <span style={{ color: C.dim }}>{bt.gf}–{bt.gc} {t("pm.inNMatches").replace("{n}", historial.length)}</span>
                      </div>
                      {contra.length > 0 ? (
                        <>
                          <div className="text-[11px] mb-1.5" style={{ color: C.dim }}>
                            {t("pm.against").replace("{rival}", eq ? eq.name : rivalTxt)} {br.g}{t("pm.win")} {br.e}{t("pm.draw")} {br.p}{t("pm.loss")} ({br.gf}–{br.gc})
                          </div>
                        ...
                        <div className="text-[11px]" style={{ color: C.dim }}>{t("pm.firstSaved").replace("{rival}", eq ? eq.name : rivalTxt)}</div>
                      )}
                    </>
                  );
                })()}
                  ...
                  {abierto && (
                    <div className="mt-3 pt-3 border-t" style={{ borderColor: C.line }}>
                      {h.analisis ? (
                        ...
                        <>
                          <div className="text-[11px] mb-2" style={{ color: C.dim }}>{t("pm.noAnalysisYet")}</div>
                          <button onClick={() => generarAnalisisHistorico(h)} disabled={generando}
                            ...
                            {generando ? t("pm.thinking") : t("pm.go")}
                          </button>
                        </>
                      )}
                    </div>
  ...
  const renderTraining = () => (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      {sesionesPublicadas.length > 0 && (
        <Card title={t("tr.publishedTitle")} className="lg:col-span-2">
          <div className="space-y-1.5">
            {sesionesPublicadas.map((p) => (
              <div key={p.rec} className="flex flex-wrap items-center gap-2 py-2 px-2.5 rounded-lg border"
                ...
                <div className="flex-1 min-w-[160px]">
                  <div className="text-sm" style={{ color: C.chalk }}>
                    ...
                    {p.rec === proximaSesionPublicada?.rec && (
                      <span className="ml-2 text-[10px] font-display uppercase tracking-wide px-1.5 py-0.5 rounded" style={{ background: AC, color: C.sobre }}>{t("tr.next")}</span>
                    )}
                  </div>
                  <div className="text-[11px]" style={{ color: C.dim }}>
                ...
                <button onClick={() => usarPlantilla(p)} className="text-xs px-2.5 py-1.5 rounded-lg border font-display uppercase tracking-wide"
                  style={{ borderColor: AC, color: AC }}>{t("tr.load")}</button>
              </div>
            ))}
          </div>
        </Card>
        ...
        <div className="mt-4 pt-3 border-t" style={{ borderColor: C.line }}>
          ...
          {canProposeChanges() && miPropuestaPendiente("training") ? (
            ...
            <button onClick={guardarSesion} disabled={!trainCompleta || sesBusy}
              ...
              style={{ background: AC, color: C.sobre }}>
              {sesBusy ? t("a.sending") : canProposeChanges() ? t("tr.sendProposal") : t("tr.saveSession")}
            </button>
          )}
  ...
  const renderStats = () => {
    ...
    return (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            {conFicha.length === 0 ? (
              ...
              <>
                ...
                {abierto ? (
                      ...
                      <table className="w-full text-sm" style={{ color: C.chalk }}>
                        <thead>
                          <tr className="text-[10px] font-display uppercase tracking-widest" style={{ color: C.dim }}>
                            <th className="text-left px-3 py-1.5">{t("st.colPlayer")}</th>
                            <th className="px-2 py-1.5">{t("st.colStarter")}</th>
                            <th className="px-2 py-1.5">{t("st.colGoals")}</th>
                            <th className="px-2 py-1.5">{t("st.colCards")}</th>
                            <th className="px-2 py-1.5">{t("st.colSubs")}</th>
                          </tr>
                        </thead>
                        <tbody>
                  ...
                  <div className="text-[11px]" style={{ color: C.dim }}>{t("st.tapMatch")}</div>
                )}
              </>
            )}
          </Card>
        ...
        {acumulado.length > 0 && (
          <div className="lg:col-span-2">
            <Card title={t("st.seasonTitle").replace("{n}", conFicha.length)}>
              <div className="overflow-x-auto">
                <table className="w-full text-sm" style={{ color: C.chalk }}>
                  <thead>
                    <tr className="text-[10px] font-display uppercase tracking-widest" style={{ color: C.dim }}>
                      <th className="text-left px-3 py-1.5">{t("st.colPlayer")}</th>
                      <th className="px-2 py-1.5">{t("st.colCalled")}</th>
                      <th className="px-2 py-1.5">{t("st.colStarter")}</th>
                      <th className="px-2 py-1.5">{t("st.colGoals")}</th>
                      <th className="px-2 py-1.5">{t("st.colCards")}</th>
                    </tr>
  ...
  /* Botón flotante de accesibilidad, solo en móvil: zoom de la web y brillo
     ...
     tocar los ajustes del teléfono. Vive fuera del árbol de sesión (se
     define antes del `if (!session)`) para estar disponible también en la
     pantalla de acceso, no solo dentro de la app. */
  const AccesFAB = (
    <div className="sm:hidden fixed z-40" style={{ right: 12, bottom: "calc(env(safe-area-inset-bottom, 0px) + 76px)" }}>
      {accOpen && (
        <div className="absolute bottom-14 right-0 w-64 rounded-lg border p-3" style={{ borderColor: C.line, background: C.panel, boxShadow: "0 8px 24px rgba(0,0,0,.28)" }}>
          <div className="flex items-center justify-between mb-3">
            <span className="font-display text-sm uppercase tracking-wide" style={{ color: C.chalk }}>{t("acc.title")}</span>
            <button onClick={() => setAccOpen(false)} aria-label={t("c.closeBtn")} className="text-lg leading-none px-1" style={{ color: C.dim }}>✕</button>
          </div>
          <div className="mb-3">
            <div className="text-[11px] mb-1.5" style={{ color: C.dim }}>{t("acc.zoom")}</div>
            <div className="flex items-center gap-2">
              <button onClick={() => setZoomWeb((z) => Math.max(ZOOM_MIN, z - ZOOM_STEP))} disabled={zoomWeb <= ZOOM_MIN}
                aria-label={`${t("acc.decrease")} ${t("acc.zoom")}`}
                className="w-9 h-9 rounded-lg border font-display text-base disabled:opacity-30" style={{ borderColor: C.line, color: C.chalk }}>A−</button>
              <span className="flex-1 text-center text-sm tabular-nums" style={{ color: C.chalk }}>{zoomWeb}%</span>
              <button onClick={() => setZoomWeb((z) => Math.min(ZOOM_MAX, z + ZOOM_STEP))} disabled={zoomWeb >= ZOOM_MAX}
                aria-label={`${t("acc.increase")} ${t("acc.zoom")}`}
                className="w-9 h-9 rounded-lg border font-display text-base disabled:opacity-30" style={{ borderColor: C.line, color: C.chalk }}>A+</button>
            </div>
          </div>
          <div className="mb-3">
            <div className="text-[11px] mb-1.5" style={{ color: C.dim }}>{t("acc.brightness")}</div>
            <div className="flex items-center gap-2">
              <button onClick={() => setBrilloWeb((b) => Math.max(BRILLO_MIN, b - BRILLO_STEP))} disabled={brilloWeb <= BRILLO_MIN}
                aria-label={`${t("acc.decrease")} ${t("acc.brightness")}`}
                className="w-9 h-9 rounded-lg border text-base disabled:opacity-30" style={{ borderColor: C.line, color: C.chalk }}>☀−</button>
              <span className="flex-1 text-center text-sm tabular-nums" style={{ color: C.chalk }}>{brilloWeb}%</span>
              <button onClick={() => setBrilloWeb((b) => Math.min(BRILLO_MAX, b + BRILLO_STEP))} disabled={brilloWeb >= BRILLO_MAX}
                aria-label={`${t("acc.increase")} ${t("acc.brightness")}`}
                className="w-9 h-9 rounded-lg border text-base disabled:opacity-30" style={{ borderColor: C.line, color: C.chalk }}>☀+</button>
            </div>
          </div>
          {(zoomWeb !== 100 || brilloWeb !== 100) && (
            <button onClick={() => { setZoomWeb(100); setBrilloWeb(100); }}
              className="w-full text-xs px-3 py-2 rounded-lg border font-display uppercase tracking-wide" style={{ borderColor: C.line, color: C.dim }}>
              {t("acc.reset")}
            </button>
          )}
        </div>
      )}
      <button onClick={() => setAccOpen((v) => !v)} aria-label={t("acc.button")} aria-expanded={accOpen}
        className="w-12 h-12 rounded-full border flex items-center justify-center text-xl"
        style={{ borderColor: accOpen ? AC : C.line, background: C.panel, color: accOpen ? AC : C.chalk, boxShadow: "0 4px 14px rgba(0,0,0,.24)" }}>
        ♿
      </button>
    </div>
  );
